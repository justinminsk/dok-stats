...
from daughters_stats import web
    ...
    _write_csv(
        ...
        [
            [
                ...
                "3",
                "10",
                '[{"regiment":"General\'s Regiment","name":"Witch Aelves","points":110,"models":10,"reinforced":false,"notes":["General"]}]',
            ],
            [
                "Teams",
                "April 13-19",
...
def test_stats_summary_text_surfaces_all_top_fields() -> None:
    text = web._stats_summary_text(
        result_rows=[["5-0", "10"]],
        presence_rows=[["Witch Aelves", "8", "80.0%"]],
        subfaction_rows=[["Hagg Nar", "6"]],
        unit_model_rows=[["Blood Sisters", "40"]],
    )
    assert "5-0" in text
    assert "Witch Aelves" in text
    assert "Hagg Nar" in text
    assert "Blood Sisters" in text
